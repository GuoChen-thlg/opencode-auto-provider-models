...
async function fetchRemoteModels(baseURL, apiKey, timeoutMs) {
  ...
  const promise = (async () => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`request failed: HTTP ${response.status} ${response.statusText} for ${reqUrl}`)
        throw new Error(`request failed with status ${response.status}`)
      }

      const payload = await response.json()
      if (!Array.isArray(payload?.data)) {
        console.error(`invalid response body for ${reqUrl}`, payload)
      ...
      if (error.name === "AbortError") {
        console.error(`timeout after ${timeoutMs}ms for ${reqUrl}`)
        throw new Error(`timeout after ${timeoutMs}ms`)
      }
      if (error instanceof TypeError) {
        console.error(`network error for ${reqUrl}: ${error.message}`)
      }
      throw error
    }
  })()
...
function resolveProviderEntries(pluginOptions) {
  if (!pluginOptions || typeof pluginOptions !== "object") return []
...
async function syncProviderV1(config, providerEntry, globalOpts) {
  ...
  const providerConfig = config?.provider?.[providerId] || config?.providers?.[providerId]
  if (!providerConfig || typeof providerConfig !== "object") {
    console.warn(`[auto-provider-models] provider not found in config: ${providerId}`)
    return
  }

  const baseURL = providerEntry.baseURL || normalizeBaseUrl(providerConfig?.options?.baseURL || providerConfig?.settings?.baseURL || globalOpts.baseURL)
  if (!baseURL) {
  ...
  try {
    const apiKey = getApiKey(providerConfig.options || providerConfig.settings, providerEntry, globalOpts)
    ...
    if (globalOpts.enrich) {
      enrichCache = await fetchAndBuildEnrichCache({
        onError: (msg) => console.warn(`enrich fetch failed: ${msg}`),
      })
    }
    ...
    console.warn(`failed to sync models for ${providerId}: ${message}`)
  }
}
...
async function setup(ctx) {
  ...
  // 注册 provider transform：从 syncedModels 读取并应用模型补丁
  ...
  // 异步获取模型数据，存入 syncedModels，然后 reload
  ...
  ;(async () => {
    for (const entry of entries) {
      ...
      const baseURL = entry.baseURL || normalizeBaseUrl(settings.baseURL || ctx.options.baseURL)
      if (!baseURL) {
        continue
      }

      const timeoutMs = Number.isFinite(ctx.options.timeout) ? ctx.options.timeout : DEFAULT_TIMEOUT
      const apiKey = getApiKey(settings, entry, ctx.options)
      ...
      try {
        ...
        for (const remoteModel of remoteModels) {
          const modelID = normalizeModelId(remoteModel?.id)
          if (!modelID || !shouldKeepModel(modelID, ctx.options)) continue
          items.push({ modelID, patch: buildV2ModelPatch(modelID, remoteModel, null, enrichCache) })
        }
...
/**
 * v2 effect 函数：供 OpenCode v2 调用（Effect-ts）
 */
function effect(ctx) {
  return Effect.gen(function* () {
    // effect 模式不使用，setup 是主要入口
  })
}
